...
import { findProjectRoot, exists, readText, readJSON } from '../shared/fs'
...
async function resolveNodeVersion(projectRoot: string): Promise<string | null> {
  ...
  if (await exists(nvmrc)) {
    try { return (await readText(nvmrc)).trim() } catch { /* unreadable */ }
  }
  try {
    const pkg = await readJSON<{ engines?: { node?: string } }>(path.join(projectRoot, 'package.json'))

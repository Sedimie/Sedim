...
const PROVIDER_CONFIG: Record<OAuthProvider, { label: string; svg: string }> = {
  google: {
    label: 'Continue with Google',
    svg: `<svg viewBox="0 0 24 24" aria-hidden="true" style="width:18px;height:18px;flex-shrink:0">
  ...
  github: {
    ...
    svg: `<svg viewBox="0 0 24 24" aria-hidden="true" style="width:18px;height:18px;flex-shrink:0;fill:currentColor">
...
export function OAuthButton({ provider, label, disabled }: OAuthButtonProps) {
  ...
  return (
    <button
      ...
      dangerouslySetInnerHTML={{ __html: `${config.svg}${label ?? config.label}` }}
    />
  )
}